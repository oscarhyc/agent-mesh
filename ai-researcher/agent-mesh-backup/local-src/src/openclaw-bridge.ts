...
// ---------------------------------------------------------------------------
// Minimal shared types (avoid external dependency)
// ---------------------------------------------------------------------------

/** Minimal AcpMessage - worker's WebSocket sends these */
interface AcpMessage { type: string; [key: string]: unknown }
/** Minimal AgentInfo - returned from registry discover */
interface AgentInfo { agentId: string; host: string; port: number; capabilities?: { agentType?: string } }
/** Internal event for the async generator queue */
...
// ---------------------------------------------------------------------------
// Mesh Runtime — spawn + async event generator
// ---------------------------------------------------------------------------
...
export class MeshRuntime {
  ...
  constructor(private config: { registry: string }) {
...
class MeshBridgeRuntime {
  ...
  async ensureSession(input: { sessionKey: string; agent: string }): Promise<any> {
  ...
  async *runTurn(input: { handle?: { sessionKey?: string }; text?: string; attachments?: any[] }): AsyncIterable<any> {
...
export async function registerMeshBridge(config?: { registry?: string; defaultAgentType?: string }): Promise<void> {
