    ...
    case "test-task": {
      ...
      const result = await new Promise<string>((resolve) => {
        const msgHandler = (data: Buffer) => {
          ...
          // Worker now sends { type: "result", taskId, final } after ACP handshake completes
          if (msg["type"] === "result" && msg["final"]) {
            workerWs.off("message", msgHandler);
            resolve(String(msg["final"]));
          }
          // Handle streaming chunks (TC4 - real-time output)
          if (msg["type"] === "chunk") {
            process.stdout.write(String(msg["text"] ?? ""));
          }
        };
        workerWs.on("message", msgHandler);
        setTimeout(() => { workerWs.off("message", msgHandler); resolve("timeout"); }, 60_000);
      });
